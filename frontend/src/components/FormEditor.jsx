...
const FormEditor = ({ user, onLogout, onToggleSidebar, onSaveForm, onCancel, form, navigateToDashboard }) => {
  ...
  const handleDownloadPDF = () => {
    ...
    const absolutePdfPath = `/Users/jibranhutchins/Developer/conform/backend/html_outputs/filled_pdfs/filled_form_5.pdf`;
    
    // Try multiple approaches to open the PDF
    
    // 1. Try using the file:// protocol (may be blocked by browser security)
    try {
      window.open(`file://${absolutePdfPath}`, '_blank');
    } catch (error) {
      console.error("Error opening PDF with file:// protocol:", error);
    }
    
    // 2. Also try the API endpoint as a fallback
    window.open(`/api/pdf-preview?path=${encodeURIComponent(absolutePdfPath)}`, '_blank');
    
    // 3. Show the path to the user in case the automatic methods fail
